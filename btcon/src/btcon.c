...
void* btcon_app_malloc() {
    ...
    view_dispatcher_set_custom_event_callback(app->view_dispatcher, btcon_custom_event_callback);
    view_dispatcher_set_navigation_event_callback(
        app->view_dispatcher, btcon_cusom_back_event_callback);
    view_dispatcher_set_tick_event_callback(app->view_dispatcher, btcon_tick_event_callback, 100);

    app->main_menu = submenu_alloc();
    view_dispatcher_add_view(
        app->view_dispatcher, BtconMainMenuView, submenu_get_view(app->main_menu));
...
void btcon() {
    ...
    app->hid_profile =
        bt_profile_start(app->bt, ble_profile_hid, (void*)&btcon_ble_profile_params);
